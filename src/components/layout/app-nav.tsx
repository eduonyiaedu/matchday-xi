...
export function AppNav({
  ...
  return (
    <>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-6 border-t border-white/9 bg-pitch/95 px-1 pt-2.5 pb-4 md:hidden">
        {LINKS.map((link) => {
          const active = pathname.startsWith(link.href);
          ...
          return (
            <Link key={link.href} href={link.href} className="flex flex-col items-center gap-1">
              ...
              />
              <span
                className="font-heading text-[9px] tracking-[0.06em] uppercase"
                style={{ color }}
              >
                {link.shortLabel}
              </span>
